...
public class FaveAmp extends AppCompatActivity implements FaveAmpAdapter.OnItemClickListener {

    private RecyclerView recyclerView;
    private DatabaseReference databaseReference;
    private StorageReference storageReference;
    private List<FaveGet> myList;
    private FaveAmpAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fave_amp);
        ...
        adapter = new FaveAmpAdapter(myList, databaseReference, storageReference, listener);
    ...
    private void fetchDataAndPopulateRecyclerView() {
        if (isConnected()) {
            ...
            DatabaseReference favoritesReference = FirebaseDatabase.getInstance().getReference().child("user").child(userId).child("Favorites");
            ...
            favoritesReference.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        String ampKey = snapshot.getKey();

                        databaseReference.child(ampKey).addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                FaveGet myGet = snapshot.getValue(FaveGet.class);
                                if (myGet != null) {
                                    myList.add(myGet);
                                }

                                // Sort the list by the "date" field
                                Collections.sort(myList, new Comparator<FaveGet>() {
                                    @Override
                                    public int compare(FaveGet o1, FaveGet o2) {
                    ...
                    if (!dataSnapshot.exists()) {
                        loadingDialog.dismiss();
                        swipeRefreshLayout.setRefreshing(false);
                        Toast.makeText(FaveAmp.this, "No favorites found", Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    @Override
    public void onItemClick(FaveGet item) {
                            ...
                            if (settingsSnapshot.exists()) {
                                ...
                                itemFolderRef.child("Effects").addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot effectsSnapshot) {
                                        ...
                                        Intent intent = new Intent(FaveAmp.this, AmpView.class);
